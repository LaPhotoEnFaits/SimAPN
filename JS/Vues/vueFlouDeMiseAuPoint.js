...
function drawVueFlouDeMiseAuPoint() {
	...
	if (vueFlouDeMiseAuPoint.visible) {
		...
		var m2px = (LARGEUR_CVS_FDMAP - MARGE_GAUCHE_CVS_FDMAP - MARGE_DROITE_CVS_FDMAP) / (1.25 * distanceMax);
		var flou2px = (HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP - MARGE_HAUT_CVS_FDMAP) / vueFlouDeMiseAuPoint.hauteurEnMetre;
		var largeurAxeX = Math.round(1.25 * distanceMax * m2px);
		...
		demiFlecheHorizontale(ct, HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP - 1, X0, X0 + largeurAxeX, COLOR_AXES_FDMAP);
		demiFlecheVerticale(ct, X0, HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP - 1, MARGE_HAUT_CVS_FDMAP, COLOR_AXES_FDMAP);

		//VALEURS DES DISTANCES ET TRAITS VERTICAUX
		for (var i = 0; i != 3; i++) {
			ct.fillStyle = COULEURS_SUJETS[i];
			ct.fillText(scene.plans[i].distance.toFixed(2) + 'm', X0 + Math.round(scene.plans[i].distance * m2px) - 10, HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP + 10 * (i + 1));
			var flou = calcFlou(scene.plans[i].distance).flou;
			flou = flou * flou2px;
			ct.beginPath();
			ct.fillRect(X0 + Math.round(scene.plans[i].distance * m2px), Y0 - flou, 1, flou);
		}
		...
		var Xdebut = X0 + m2px * priseDeVue.debutDeLaPDC;
		var Xfin = X0 + m2px * priseDeVue.finDeLaPDC;
		...
		ct.fillStyle = "#333333";
		flecheVerticale(ct, X0 + largeurAxeX + 5, HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP, HAUTEUR_CVS_FDMAP - MARGE_BAS_CVS_FDMAP - tailleEchelle * apnChoisi.taillePixel * flou2px, '#333333');
...
function deplacementSourisVueFlouDeMiseAuPoint(e) {
	...
	for (var i = 0; i != 3; i++)
		Xplan[i] = m2px * scene.plans[i].distance;
	...
	if (flagClicVueFlouDeMiseAuPoint) {
		...
		scene.plans[planSelectionneTemporaire].distance += (X1VueFlouDeMiseAuPoint - X0VueFlouDeMiseAuPoint) / m2px;

		if (planSelectionneTemporaire === 0) {			
			if (scene.plans[0].distance > scene.plans[1].distance)
				scene.plans[0].distance = scene.plans[1].distance;
		}
		
		if (planSelectionneTemporaire === 2) {
		...
		if (planSelectionneTemporaire === 1) {
		...
		onModifDistancePlan(planSelectionneTemporaire);
	}
